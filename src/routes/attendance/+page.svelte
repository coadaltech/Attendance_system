  ...
  {:else}
    ...
    {#if summary}
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {#each [
          { label: 'Working Days', value: summary.totalWorkingDays, color: 'text-brand-700 bg-brand-50' },
          { label: 'Present', value: summary.presentDays, color: 'text-emerald-700 bg-emerald-50' },
          { label: 'Absent', value: summary.absentDays, color: 'text-red-700 bg-red-50' },
          { label: 'Attendance %', value: `${summary.attendancePercent}%`, color: 'text-purple-700 bg-purple-50' },
        ] as s}
          ...
          <tbody class="divide-y divide-gray-50">
            {#each history as record}
              ...
              <tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-4 py-3 text-sm text-gray-600">{formatTime(record.punchIn)}</td>
                <td class="px-4 py-3 text-sm text-gray-600">{formatTime(record.punchOut)}</td>
                <td class="px-4 py-3 text-sm text-gray-600">{record.workingHours ? formatHours(record.workingHours) : '-'}</td>
                <td class="px-4 py-3">
                  <span class="{badge.class}">{badge.label}</span>
                </td>
                <td class="px-4 py-3 text-xs text-gray-400">{record.notes || '-'}</td>
              </tr>
            {:else}
              <tr>
                <td colspan="6" class="px-4 py-8 text-center text-gray-400 text-sm">No attendance records for this period</td>
              </tr>
            {/each}
          </tbody>
        </table>
